...
export class ComprarProductoComponent implements OnInit {
  ...
  formularioComprar = new FormGroup({
    cantidad: new FormControl('', Validators.required)
  });

  comprar(cantidad: string){
    let cant = +cantidad;
    let precioTotal = cant*(this.planetaProducto.factorOferta / (1 + this.planetaProducto.stock));
    //Transaccion
    let transaccion: Transaccion = new Transaccion(0,precioTotal, 0, "compra", this.usuario);
